...
export default function Home() {
  ...
  async function handleContact(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Formspree no aceptó la solicitud.");

      formElement.reset();
      setFormNotice("Solicitud enviada. Si no recibe respuesta, escriba directamente a firmabordados@yahoo.com.");
    } catch {
      setFormNotice("No fue posible enviar la solicitud. Use el correo oficial como alternativa o inténtelo de nuevo más tarde.");
    } finally {
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#fffdf7] text-[#183d79]">
      ...
      <main id="contenido-principal" ref={mainRef} tabIndex={-1}>
        ...
        <section id="contacto" className="relative overflow-hidden bg-[#0d4c9e] py-20 text-white lg:py-28">
          ...
          <div className="relative z-10 mx-auto grid max-w-[1440px] gap-12 px-5 lg:grid-cols-[0.85fr_1.15fr] lg:px-10"><div data-reveal><SectionLabel number="04" label="Contacto" dark /><h2 className="mt-5 font-display text-4xl font-bold leading-tight tracking-[-0.04em] sm:text-5xl">Conversemos sobre lo que necesita su equipo.</h2><p className="mt-6 max-w-md text-base leading-7 text-white/82">Comparta brevemente su necesidad o comuníquese directamente con Firma Bordados.</p><div className="mt-6 flex items-center gap-3"><StitchRule className="text-[#ffe584]" /><span className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-white/65">Respuesta clara · trato cercano</span></div><div className="mt-9 space-y-5 text-sm"><a href="https://wa.me/528787880735?text=Hola%2C%20me%20gustar%C3%ADa%20solicitar%20informaci%C3%B3n%20sobre%20prendas%2C%20bordado%20o%20serigraf%C3%ADa." target="_blank" rel="noreferrer" className="flex items-start gap-3 text-white/90 transition-colors hover:text-[#ffe584]"><MessageCircle className="mt-0.5 text-[#ffe584]" size={18} /><span>WhatsApp · (878) 788-0735</span></a><a href="mailto:firmabordados@yahoo.com" className="flex items-start gap-3 text-white/90 transition-colors hover:text-[#ffe584]"><Mail className="mt-0.5 text-[#ffe584]" size={18} /><span>firmabordados@yahoo.com</span></a><a href="tel:+528787822024" className="flex items-start gap-3 text-white/90 transition-colors hover:text-[#ffe584]"><Phone className="mt-0.5 text-[#ffe584]" size={18} /><span>(878) 782-2024</span></a><a href="tel:+528787834378" className="flex items-start gap-3 text-white/90 transition-colors hover:text-[#ffe584]"><Phone className="mt-0.5 text-[#ffe584]" size={18} /><span>(878) 783-4378</span></a><div className="flex items-start gap-3 text-white/90"><MapPin className="mt-0.5 text-[#ffe584]" size={18} /><span>Emilio Carranza #1021 Int. 113<br />Col. Burócratas, Piedras Negras, Coahuila.</span></div><div className="flex items-start gap-3 text-white/90"><Clock3 className="mt-0.5 text-[#ffe584]" size={18} /><span>Lunes a viernes · 8:00 am a 6:30 pm</span></div></div><figure className="mt-9 max-w-sm overflow-hidden rounded-2xl border border-white/20 bg-white/10 shadow-[0_16px_35px_rgba(0,0,0,0.18)]"><img src={storefrontImage} alt="Fachada de Firma Bordados en Piedras Negras" loading="lazy" decoding="async" className="aspect-square w-full object-cover" /><figcaption className="flex items-center gap-2 px-4 py-3 text-[0.65rem] font-bold uppercase tracking-[0.12em] text-white/85"><MapPin size={14} className="text-[#ffe584]" /> Atención local en Piedras Negras</figcaption></figure></div><form data-reveal data-reveal-delay="1" onSubmit={handleContact} className="rounded-2xl bg-white p-6 text-[#183d79] shadow-[0_20px_45px_rgba(0,0,0,0.2)] sm:p-9"><input type="hidden" name="_subject" value="Nueva solicitud — Firma Bordados" /><input type="text" name="_gotcha" className="sr-only" tabIndex={-1} autoComplete="off" aria-hidden="true" /><div className="flex items-center justify-between border-b border-[#d8e0ec] pb-5"><div><span className="text-[0.68rem] font-bold uppercase tracking-[0.15em] text-[#0d4c9e]">Solicita información</span><p className="mt-1 text-xs text-[#687b98]">Cuéntanos qué necesitas y te responderemos desde Firma Bordados.</p></div><img src={officialLogo} alt="" loading="lazy" decoding="async" className="h-9 w-11 object-contain" /></div><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Nombre" name="name" placeholder="Tu nombre" required /><Field label="Correo electrónico" name="email" placeholder="tu@empresa.com" type="email" required /><Field label="Empresa (opcional)" name="company" placeholder="Nombre de tu empresa" /><Field label="Teléfono (opcional)" name="phone" placeholder="Tu teléfono" type="tel" /></div><div className="mt-5"><label htmlFor="message" className="text-xs font-bold uppercase tracking-[0.12em] text-[#4d6183]">¿Qué necesitas?</label><textarea id="message" name="message" required rows={5} placeholder="Cuéntanos sobre tus prendas, bordado, serigrafía o cantidad aproximada." className="mt-2 w-full resize-none rounded-md border border-[#cbd8eb] bg-[#f8fbff] px-4 py-3 text-sm outline-none transition-colors placeholder:text-[#8495ae] focus:border-[#0d4c9e] focus:ring-2 focus:ring-[#0d4c9e]/20" /></div><button type="submit" disabled={isSubmitting} className="motion-cta mt-6 inline-flex w-full items-center justify-center gap-3 rounded-md bg-[#df2b2c] px-5 py-4 text-xs font-bold uppercase tracking-[0.13em] text-white transition-all hover:bg-[#c82021] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70">{isSubmitting ? "Enviando solicitud…" : "Enviar solicitud"} <Send size={16} /></button><p className="mt-4 text-xs leading-5 text-[#687b98]">También puedes escribir directamente a <a className="font-bold text-[#0d4c9e] underline decoration-[#f3bd25] underline-offset-4" href={`mailto:${officialEmail}`}>{officialEmail}</a>.</p><a href={guidedEmailUrl} className="mt-3 inline-flex text-xs font-bold text-[#0d4c9e] underline decoration-[#f3bd25] decoration-2 underline-offset-4 hover:text-[#df2b2c]">Preparar correo oficial</a>{formNotice && <p className="mt-3 rounded-md border-l-4 border-[#f3bd25] bg-[#fff7da] px-3 py-2 text-xs font-medium text-[#765400]">{formNotice}</p>}</form></div>
        </section>
      </main>
